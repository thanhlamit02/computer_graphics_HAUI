...
{
	instance = Scale(w, h, l);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * instance_ban * instance * model_view);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void ban()
{
	...
	matban(0.6, 0.4, 0.05);
	instance_ban = Translate(0.29, -0.15, 0.19);
	chanban(0.02, 0.3);
	instance_ban = Translate(-0.29, -0.15, 0.19);
	chanban(0.02, 0.3);
	instance_ban = Translate(-0.29, -0.15, -0.19);
	...
	instance_ban = Translate(0.29, -0.15, -0.19);
	chanban(0.02, 0.3);
	instance_ban = Translate(0, -0.06, 0);
}
void initGPUBuffers(void)
{
...
void display(void)
{
	...
	ban();
	model_view = model_view * Translate(0, 0, -0.15);
	theta[0];
	ngankeo(0.4, 0.3, 0.01);
	glutSwapBuffers();
}
void keyboard(unsigned char key, int x, int y)
{
